...
class App extends Component {
  ...
  getJSON() {
    ...
    let currentUnitPrice = parseFloat(products[i].unit_cost.slice(1));
    return(
      fetch("https://api.myjson.com/bins/195yz8")
      .then((response) => response.json())
      .then((productsJSON) => {
        this.setState({
          productName: productsJSON[i].product_name,
          quantity: productsJSON[i].quantity,
          unitPrice: currentUnitPrice,
          subTotal: productsJSON[i].quantity*currentUnitPrice,
